...
def load_fonts():
    try:
        # Gujarati Bold
        font_url = "https://github.com/itfoundry/hind-vadodara/raw/master/fonts/ttf/HindVadodara-Bold.ttf"
        font_response = requests.get(font_url)
        with open("GujFont-Bold.ttf", "wb") as f:
            f.write(font_response.content)
            
        # Gujarati Regular
        font_url_reg = "https://github.com/itfoundry/hind-vadodara/raw/master/fonts/ttf/HindVadodara-Regular.ttf"
        font_response_reg = requests.get(font_url_reg)
        with open("GujFont-Reg.ttf", "wb") as f:
            f.write(font_response_reg.content)
            
        pdfmetrics.registerFont(TTFont('GujBold', 'GujFont-Bold.ttf'))
        pdfmetrics.registerFont(TTFont('GujReg', 'GujFont-Reg.ttf'))
        return True
...
# --- SIDEBAR SETTINGS ---
st.sidebar.image("https://upload.wikimedia.org/wikipedia/commons/thumb/c/c3/Python-logo-notext.svg/1200px-Python-logo-notext.svg.png", width=50) # Logo placeholder
...
st.markdown("તમારું **Question Paper PDF**, **Answer Key CSV**, અને **Background Image** અપલોડ કરો.")
...
with col3:
    img_file = st.file_uploader("3. Background (JPG/PNG)", type=['png', 'jpg', 'jpeg'])

# --- PROCESSING ---
if st.button("Generate PDF 🚀"):
    if pdf_file and csv_file and img_file:
        try:
            with st.spinner("Processing... Please wait"):
                ...
                for i in range(num_cols_needed):
                    col_idx_no = i * 2
                    col_idx_ans = i * 2 + 1
                    style.add('FONTNAME', (col_idx_no, 1), (col_idx_no, -1), 'GujBold')
                    ...
                    style.add('FONTNAME', (col_idx_ans, 1), (col_idx_ans, -1), 'GujReg')
                ...
                # 4. MERGE
                reader_main = PdfReader(pdf_file) # Re-read for fresh stream
        ...
        st.warning("⚠️ Please upload all 3 files first.")
